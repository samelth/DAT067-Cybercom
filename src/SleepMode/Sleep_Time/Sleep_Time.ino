...
void connect(){
 ...
 void setupPsm(String TAU, String AT){
  String response;
  ...
  MODEM.send("AT+UPSMR=1");
  MODEM.waitForResponse(2000, &response);
  Serial.print(response);
  delay(2000);
  
  MODEM.sendf("AT+CPSMS=1,,,\"%s\",\"%s\"", TAU.c_str(), AT.c_str());
  MODEM.waitForResponse(2000, &response);
  Serial.print(response);
  delay(2000);
